...
# Product Update
    ...
    for update_df in chunked_df:
        ...
        extracted_variant_ids = [{'product_id': x['node']['product']['id'], 'variant_id': x['node']['id']} for x in variant_ids]
        variant_id_df = pd.DataFrame.from_records(extracted_variant_ids)
        variant_id_df.to_csv('data/update_product_vids.csv', index=False)

        # ========================================Fill create product_id =================================================
        fill_variant_id(update_df, product_id_filepath='data/update_product_vids.csv', mode='update')

        # =====================================Bulk update Shopify variant================================================
        csv_to_jsonl(csv_filename='data/update_product_variants_with_vids.csv', jsonl_filename='bulk_op_vars.jsonl', mode='vup')
